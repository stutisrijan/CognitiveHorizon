...
class Level1:
    SIMPLE_WORDS = ["apple", "ball", "cat", "dog", "egg", "fish", "go", "hat", "ink", "jug"]
    ...
    def init_ui(self):
        ...
        self.label = tk.Label(self.frame, text="Level 1: Form a word!", font=("Arial", 14), bg="white")
    ...
    def check_word(self):
        ...
        if set(word).issubset(set(self.letters)):
            self.cancel_timer()
            self.update_score_callback(len(word))
            self.current_question += 1
            self.entry.delete(0, tk.END)
            self.new_game()
        else:
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    root.title("Word Builder - Level 1")
    ...
    level = Level1(root, canvas, dummy_next_level, dummy_update_score, dummy_generate_report, dummy_back_to_menu)
